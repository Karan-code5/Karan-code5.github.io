...
const Hero = ({ profile }) => {
  // Safe handling of experience string
  const experienceText = profile.totalExperience || "1+ YEAR EXPERIENCE";
  const experienceYears = experienceText.split(' ')[0] || "1+";

  return (
    <section id="identity" className="hero-container">
      <div className="hero-content">
        <div className="status-tag">
          STATUS: OPERATIONAL // XP_LEVEL: {experienceText} // SECURITY_LEVEL: ENGINEER
        </div>
        <h1 className="big-heading">{profile.name}</h1>
        <h2 className="sub-heading">Security Engineer | EDR & XDR Specialist</h2>

        <div className="terminal-box">          <p className="primary-text">&gt; Initializing mission_statement...</p>
          <p className="bio">
